...
def write_dataset(args):
    ...
    pattern = os.path.join(args.shards, f'ego4d-EPIC-{args.split}-%06d.tar')
    video_to_path = {}
    path_to_class = {}
    annotations_data = pd.read_csv(open(f'/home/username/epic-kitchens/annotations/EPIC_100_{args.split}.csv'))
    video_files_path = "/home/username/epic-kitchens/clips/*"
    ...
    for i, clip_data in annotations_data.iterrows():
        ...
        clip_id = clip_data['narration_id']
        video_id = clip_data['video_id']
        if clip_id not in video_to_path:
            print(f'Skipped {clip_id}')
            continue
        path_to_class[video_to_path[clip_id]] = {'noun': clip_data['noun_class'], 'verb' : clip_data['verb_class']}
    ...
    with wds.ShardWriter(
        ...
        for idx, video_file in tqdm(enumerate(path_to_class.keys()), desc='total', total=len(path_to_class)):
            ...
            sample = {'__key__': xkey, 'mp4': video_data, 'json': path_to_class[video_file] }
