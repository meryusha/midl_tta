...
https://github.com/facebookresearch/mae_st/blob/main/util/decoder/mixup.py,
...
class MixUp:
    ...
    def __init__(
        ...
        Args:
            ...
            num_classes (list): Number of classes for targets.
        """
        ...
        #cutmix not implemented for audiovisual, thus make sure is not active
        if len(self.modalities) > 1:
            assert self.cutmix_alpha == 0.0, "cutmix not implemented for multimodal data"
    ...
    def __call__(self, x, target):
        assert len(x[self.modalities[0]].shape) > 1, "Batch size should be greater than 1 for mixup."
        lam = self._mix_batch(x)
        
        for this_num_classes, (target_name, this_target) in zip(self.num_classes, target.items()):
            target[target_name] = mixup_target(this_target, this_num_classes, lam, self.label_smoothing)
        return x, target